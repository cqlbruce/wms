    ...
    <div style="width: 100%;">
      <el-form ref="analysisForm" :model="stockStatistics" :label-position="labelPosition" :inline="true" size="medium">
        <el-row>
          ...
          <el-col style="width: 300px">
            <el-form-item label="当月入仓总体积">{{ stockStatistics.currentMonthVolume }}</el-form-item>
          </el-col>
        </el-row>
      </el-form>
    </div>
    ...
    <el-dialog title="详情" :visible.sync="detailVisible" :close-on-click-modal="false" width="80%" destroy-on-close>
      ...
      >
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="6">
            <el-form-item label="入仓报关总净重:">{{ detailTemp.custsDeclaAllWeigh }}</el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="10">
          <el-col :span="6">
            <el-form-item label="实收总毛重:">{{ detailTemp.gwAllActul }}</el-form-item>
          </el-col>
          <el-col :span="6">
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="6">
            <el-form-item label="总出仓净重:">{{ detailTemp.shippedWeigh }}</el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="总出仓毛重:">{{ detailTemp.shippedGw }}</el-form-item>
          </el-col>
          <el-col :span="6">
        ...
        <el-row :gutter="10">
          ...
          <el-col :span="6">
            <el-form-item label="总库存净重:">{{ detailTemp.stockWeigh }}</el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item label="总库存毛重:">{{ detailTemp.stockGw }}</el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="10">
          <el-col :span="6">
    ...
    handleDownload() {
      ...
      exportStockGoodsList(this.listQuery).then(response => {
        ...
        import('@/vendor/Export2Excel').then(excel => {
          const tHeader = ['客户', '项目', '收货日期', '入仓落货纸号', '入仓号', '订单号', '货物款号', '商品名称', '进仓箱数', '进仓件数', '成交单位', '入仓报关总净重', '实收总毛重', '进仓体积', '已出仓箱数', '已出仓件数', '已出仓体积', '总出仓净重', '总出仓毛重', '库存箱数', '库存件数', '库存体积', '总库存净重', '总库存毛重', '库位', '备注']
          const filterVal = ['custShortName', 'projectName', 'rcvdDate', 'so', 'inboundNo', 'po', 'item', 'productName', 'rcvdCtns', 'rcvdPcs', 'declaUnit', 'custsDeclaAllWeigh', 'gwAllActul', 'boxAllVolumeActul', 'shippedCtns', 'shippedPcs', 'shippedVolume', 'shippedWeigh', 'shippedGw', 'stockCtns', 'stockPcs', 'stockVolume', 'stockWeigh', 'stockGw', 'warehousePosition', '']
